...
exports.bufferFinished = exports.staticTransitionOut = exports.prepareOut = exports.prepareIn = exports.getBoundingClientRect = exports.remapPropsToTranslate = exports.createTranslateProp = exports.swapValues = exports.isZero = void 0;
...
const swapValues = (direction, value) => {
...
const createTranslateProp = (options = {
  direction: "right",
  from: "100%",
  to: 0
}) => {
  ...
  const translateProp = direction === "right" || direction === "left" ? "translateX" : "translateY";
  return {
...
exports.createTranslateProp = createTranslateProp;

const remapPropsToTranslate = (props, values) => {
  const {
    direction
  } = props;
  const [from, to] = values;
  return createTranslateProp({
    direction,
    from,
    to,
    props: {}
...
exports.remapPropsToTranslate = remapPropsToTranslate;

const getBoundingClientRect = node => {
  const _document = node.ownerDocument || document;

  const body = _document.body; // Cache body overflow

  const _overflow = body.style.overflow; // Temporarily lock body overflow

  body.style.overflow = "hidden"; // Get the most accurate BCR

  const rect = node.getBoundingClientRect(); // Unlock body overflow
...
exports.prepareIn = prepareIn;

const prepareOut = node => {
...
exports.prepareOut = prepareOut;

const staticTransitionOut = props => {
...
exports.staticTransitionOut = staticTransitionOut;

const bufferFinished = () => {
  return staticTransitionOut({
    duration: 100
  });
};

exports.bufferFinished = bufferFinished;